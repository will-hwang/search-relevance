...
public class RestSearchSearchConfigurationActionTests extends OpenSearchTestCase {
    private RestSearchSearchConfigurationAction restAction;
    ...
    public void setup() throws IOException {
        ...
        restAction = new RestSearchSearchConfigurationAction(settingsAccessor);
        ...
        doAnswer(invocation -> {
            ...
            return null;
        }).when(client).execute(eq(SearchSearchConfigurationAction.INSTANCE), any(), any());
    }
    ...
    public void testGetNameAndRoutes() {
        ...
        assertEquals("search_search_configuration_action", actionName);

        List<RestHandler.Route> routes = restAction.routes();
        assertNotNull(routes);
        assertEquals(2, routes.size());
        assertThat(routes.get(0).getPath(), Matchers.is(SEARCH_CONFIGURATIONS_URL + "/_search"));
    ...
    public void testHandleRequest_executesTransportAndReturnsResponse() throws Exception {
        RestRequest request = new FakeRestRequest.Builder(xContentRegistry).withMethod(RestRequest.Method.POST)
            .withPath(SEARCH_CONFIGURATIONS_URL + "/_search")
        ...
        verify(client, times(1)).execute(eq(SearchSearchConfigurationAction.INSTANCE), requestCaptor.capture(), any());
    ...
    public void testHandleRequest_whenWorkbenchDisabled_returnsForbidden() throws Exception {
        ...
        verify(client, never()).execute(eq(SearchSearchConfigurationAction.INSTANCE), any(), any());
        ArgumentCaptor<BytesRestResponse> responseCaptor = ArgumentCaptor.forClass(BytesRestResponse.class);
        verify(channel, times(1)).sendResponse(responseCaptor.capture());
        assertEquals(RestStatus.FORBIDDEN, responseCaptor.getValue().status());
    }

    public void testHandleRequest_whenNoContent_appliesDefaultSize() throws Exception {
        RestRequest request = new FakeRestRequest.Builder(NamedXContentRegistry.EMPTY).withMethod(RestRequest.Method.GET)
            .withPath(SEARCH_CONFIGURATIONS_URL + "/_search")
            .build();
    ...
    private SearchResponse buildSearchResponse() {
        ...
        hit.sourceRef(new BytesArray("{\"id\":\"config-1\"}"));
