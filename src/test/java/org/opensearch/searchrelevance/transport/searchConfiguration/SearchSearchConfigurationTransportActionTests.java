...
public class SearchSearchConfigurationTransportActionTests extends OpenSearchTestCase {
    ...
    private SearchSearchConfigurationTransportAction transportAction;

    @Before
    public void setup() {
        MockitoAnnotations.openMocks(this);
        transportAction = new SearchSearchConfigurationTransportAction(transportService, actionFilters, searchConfigurationDao);
    }
    ...
    public void testDoExecute_withSourceBuilder_callsDaoAndResponds() {
        ...
        doAnswer(invocation -> {
            ...
            return null;
        }).when(searchConfigurationDao).listSearchConfiguration(eq(builder), any());

        transportAction.doExecute(null, request, listener);

        verify(searchConfigurationDao).listSearchConfiguration(eq(builder), any());
    ...
    public void testDoExecute_withoutSourceBuilder_constructsDefault() {
        ...
        doAnswer(invocation -> {
            ...
            l.onResponse(searchResponse);
            return null;
        }).when(searchConfigurationDao).listSearchConfiguration(any(SearchSourceBuilder.class), any());

        transportAction.doExecute(null, request, listener);

        ArgumentCaptor<SearchSourceBuilder> captor = ArgumentCaptor.forClass(SearchSourceBuilder.class);
        verify(searchConfigurationDao).listSearchConfiguration(captor.capture(), any());
    ...
    public void testDoExecute_whenDaoThrows_returnsFailure() {
        ...
        doThrow(new RuntimeException("boom")).when(searchConfigurationDao).listSearchConfiguration(eq(builder), any());
